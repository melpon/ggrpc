...
class ClientReaderWriter {
  ...
  struct SafeDeleter {
    ...
    ~SafeDeleter() {
      bool del =
          ...
          p->nesting_ == 0;
      SPDLOG_TRACE(
          "del={}, release={}, read_status={}, write_status={}, nesting={}",
          del, p->release_, (int)p->read_status_, (int)p->write_status_,
          p->nesting_);
      lock.unlock();
      if (del) {
        delete p;
 ...
 private:
  void Release() {
    ...
    SPDLOG_TRACE("Release: read_status={}, write_status={}, nesting={}",
                 (int)read_status_, (int)write_status_, nesting_);
    release_ = true;
 ...
 private:
  ...
  void Done(std::unique_lock<std::mutex>& lock) {
    ...
    SPDLOG_TRACE("Done: read_status={}, write_status={}, nesting={}",
                 (int)read_status_, (int)write_status_, nesting_);
    ...
    if (read_status_ == ReadStatus::CANCELING ||
      ...
      if (need_callback_) {
        RunCallback(d.lock, "OnError_ConnectCancel", on_error_,
                    ClientReaderWriterError::CONNECT_CANCEL);
      }
      Done(d.lock);
      return;
    }
    ...
    if (read_status_ == ReadStatus::CANCELING) {
      ...
      if (need_callback_) {
        RunCallback(d.lock, "OnError_ReadCancel", on_error_,
                    ClientReaderWriterError::READ_CANCEL);
      }
      Done(d.lock);
      return;
    }
    ...
    if (write_status_ == WriteStatus::CANCELING) {
      ...
      if (need_callback_) {
        RunCallback(d.lock, "OnError_WriteCancel", on_error_,
                    ClientReaderWriterError::WRITE_CANCEL);
      }
      Done(d.lock);
      return;
    }
